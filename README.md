...
- **🗂️ 个人工作台**（纯本地，无后端）：底部「工作台」标签内聚合四件套 ——
  ...
  - **🎓 408 工作台（独立标签页）**（整合「408 考研专属辅导」技能，纯本地伴侣）：底部新增独立「408」标签，承载全部 408 操作与内容。状态概览（技能调用 / 知识点 / 今日待复习 / 连续天数）+ 掌握度分布；**可编辑真题库**——内置 8 道真题开箱即用，支持自主添加 / 编辑 / 删除题目与答案，内置题受保护可一键「恢复内置真题」；移动端点题即可展开答案、调「调用技能解析」复制规范提示词并回填解析；四大技能入口（费曼答疑 / 知识索引 / 定期归纳 / 复习提醒）一键复制提示词，去新对话粘贴给 408 专家执行；快捷操作（记录知识点、抽真题、标记今日复习完成）；知识索引（按科目→章节→知识点→掌握度+下次复习），按艾宾浩斯 R1-R5（1/3/7/16/35 天）自动排复习；近期活动记录可回填 AI 结果；附「手机端技能调用指南」。
