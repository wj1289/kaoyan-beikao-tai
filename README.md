...
- **🍅 番茄专注计时**：自定义专注 / 短休息 / 长休息时长与长休息间隔；开始 / 暂停 / 重置控制，专注结束自动切换休息并继续计时；累计完成番茄数（多端同步）。
- **📅 考研倒计时**：顶部常驻 + 今日页大字号双视图，逐秒显示距考试的天 / 时 / 分 / 秒，目标日期在设置页调整。
- **🗂️ 个人工作台**（纯本地，无后端）：底部「工作台」标签内聚合四件套 ——
  - **待办清单**：增删改、勾选完成、按高/中/低优先级筛选与按优先级/添加时间/截止日排序。
  - **日程日历**：日 / 周视图切换，点日期格快速添加日程，事件按时间排序展示。
  - **随手笔记**：输入即自动保存（localStorage），刷新不丢，实时字数统计。
  - **书签收藏**：分类创建与管理，按分类筛选，点击在新标签打开，可删除分类与书签。
